...
def _swiglu_apply_weight_to_fp8_kernel(
    x_ptr, topk_w_ptr, y_ptr, y_sf_ptr,
    M, H,                                     # 运行时形状
    stride_xm, stride_xn,                     # x: (M, 2H) 的 stride
    stride_ym, stride_yn,                     # y: (M, H)  的 stride
    stride_sfm, stride_sfk,                   # y_sf: (M, H/BLOCK_K) 的 stride
    ...
    sf = tl.maximum(amax / FP8_E4M3_MAX, 1.0e-30)
    if USE_UE8M0_SCALE:
...
def _bench_cuda_events(fn, num_warmup: int = 5, num_repeat: int = 20,
                       l2_flush_gb: float = 8.0) -> float:
    """返回 fn 的中位数耗时（秒）。"""
...
def test(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    def run_fused():
        ...
        deep_gemm.fp8_mega_moe(
            y_fused, transformed_l1, transformed_l2, sym_buffer,
    ...
    def run_baseline():
        recv_x, _, recv_topk_weights, handle, _ = ep_buffer.dispatch(
            x_fp8, topk_idx=topk_idx, topk_weights=topk_weights,
            ...
            do_expand=True, use_tma_aligned_col_major_sf=False,  # SM90: row-major float SF
        )
        n = recv_x[0].size(0)

        # L1 GEMM：FP8 token @ FP8 W1 → BF16 中间激活 (gate||up 拼接)
        l1_y = torch.empty((n, intermediate_hidden * 2),
                           dtype=torch.bfloat16, device='cuda')
        deep_gemm.m_grouped_fp8_gemm_nt_contiguous(
            recv_x, l1_weights, l1_y,
    ...
    dist_print(f' > Experts: {num_topk}/{num_experts} (per-rank: {num_experts_per_rank})',
               once_in_node=True)
    dist_print(f' > Activation SF: fused L2 per-{FUSED_L2_ACT_SF_GRAN} UE8M0, '
               f'baseline L2 per-{BASELINE_L2_ACT_SF_GRAN} UE8M0 '
               f'(SM90 grouped GEMM constraint)', once_in_node=True)
    dist_print(f' > Buffer: {sym_buffer.buffer.nbytes / 2 ** 30:.3f} GiB', once_in_node=True)
    ...
    if ep_buffer is not None:
        ...
        if args.check_output_diff:
            ...
            dist_print('Output diff (fused vs legacy-per128 baseline):', once_in_node=True)
            dist_print(f' > max_abs={diff.max().item():.6e}, '
    ...
    t_fused = bench_kineto(
        ...
        barrier=lambda: ep_buffer.barrier(use_comm_stream=False)
                        if ep_buffer is not None else dist.barrier(),
        trace_path=(f'{args.dump_profile_traces}/mega_moe_hopper_rank{rank_idx}.json'
                    if args.dump_profile_traces else None),
    )
    # baseline：cuda events 中位数（tilelang.do_bench 在 H200 不一定有，统一用 events）
    t_baseline = _bench_cuda_events(run_baseline,
                                    num_warmup=args.num_warmup,
                                    num_repeat=args.num_repeat,
                                    l2_flush_gb=args.l2_flush_gb) if ep_buffer is not None else 0.0
    ...
    l1_weight_sf_bytes = (
        ...
        (hidden // WEIGHT_SF_GRAN_K) * 4
    )
    l2_weight_sf_bytes = (
        num_touched_experts *
        (hidden // WEIGHT_SF_GRAN_MN) *
        (intermediate_hidden // WEIGHT_SF_GRAN_K) * 4
    )
    l1_input_sf_bytes = num_recv_tokens * (hidden // L1_ACT_SF_GRAN) * 4
    l2_act_sf_bytes = num_recv_tokens * (intermediate_hidden // FUSED_L2_ACT_SF_GRAN) * 4
    num_hbm_bytes = (
        l1_weight_bytes + l2_weight_bytes +                         # weights (FP8)
        l1_weight_sf_bytes + l2_weight_sf_bytes +                   # weight SF (FP32)
        num_recv_tokens * hidden + l1_input_sf_bytes +              # L1 输入读 (FP8 + SF)
        num_recv_tokens * intermediate_hidden + l2_act_sf_bytes +   # L1 输出写 (FP8 + SF)
        num_recv_tokens * intermediate_hidden + l2_act_sf_bytes +   # L2 输入读 (FP8 + SF)
    ...
    legacy_label = f'{safe_div(t_baseline, t_fused):.2f}x legacy-per128' \
                   if ep_buffer is not None else '(no baseline: deep_ep unavailable)'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num-warmup', type=int, default=5,
                        help='baseline cuda events warmup')
    parser.add_argument('--num-repeat', type=int, default=20,
                        help='baseline cuda events 测时迭代')
    parser.add_argument('--l2-flush-gb', type=float, default=8.0,
                        help='baseline event 测时前用于 flush L2 的临时写入大小；0 表示关闭')
    parser.add_argument('--check-output-diff', type=int, default=0,
